...
def create_sync_dir(repo_path: str, with_code: bool = True) -> Path:
    ...
    for extra_file in get_extra_files():
        if extra_file.is_dir():
            (repo_dir / extra_file.relative_to(project_root)).symlink_to(extra_file, target_is_directory=True)
        else:
            (repo_dir / extra_file.relative_to(project_root)).symlink_to(extra_file)
