...
class OpenAIBatchClient(RemoteBatchClient):
    ...
    enqueued_limits_fn = staticmethod(openai_enqueued_limits)
    ...
    def __init__(
        ...
        *,
        ...
        self._client = AsyncOpenAI(api_key=resolved, max_retries=5, timeout=300.0)
    ...
    async def create_batch(self, shard: Path) -> str:
        ...
        logger.debug("OpenAI created batch_id=%s file_id=%s", batch.id, file_obj.id)
        return batch.id
