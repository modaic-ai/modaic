from typing import Type, Any, ClassVar, Optional, List
from pymilvus import DataType, MilvusClient
from pydantic import BaseModel
from ...databases.vector_database import VectorDatabaseConfig
from ...context.types import SerializedContext
import uuid
import numpy as np
import dspy

class MilvusVDBConfig(VectorDatabaseConfig):
    _module: ClassVar[str] = "milvus"
...
def create_collection(client: MilvusClient, collection_name: str, metadata_schema: Type[BaseModel]) -> Any:
    """
...
def _convert_pydantic_to_milvus_schema(client: MilvusClient, pydantic_schema: Type[BaseModel]) -> Any:
    """
...
def search(client: MilvusClient, collection_name: str, vector: np.ndarray | List[int], k: int = 10, filter: Optional[dict] = None, **kwargs) -> List[SerializedContext]:
    """
...
class MilvusVectorDatabase:
    def __init__(self, config: MilvusVDBConfig, embedder: dspy.Embedder, metadata_schema: Type[BaseModel] = None, **kwargs):
        self.config = config
