...
class SerializedContext:
    ...
    def schema(self):
        primitives = {int, str, float, bool, list, dict, tuple, set, Source}
        init_keys = set(k for k, v in self.__init__.__annotations__.items())
        public_attributes = set(
            k
            for k, v in self.__dict__.items()
            if not k.startswith("_") and type(v) in primitives
        )
        hidden_attributes = set(
            k[1:]
            for k, v in self.__dict__.items()
            if k.startswith("_") and type(v) in primitives and not k.startswith("__")
        )

        if init_keys <= (public_attributes | hidden_attributes):
            warnings.warn(
                f"""class {self.__name__} is missing the following attributes which are params in __init__:
                {init_keys ^ (public_attributes | hidden_attributes)}
                This can cause unexpected behavior when serializing and deserializing the class.
                """
            )
        # print(public_attributes)
        dict_schema = {k: (type(k), ...) for k in public_attributes}
        return create_model(
            f"{self.__class__.__name__} Model", **dict_schema, __base__=BaseModel
        )
...
class Context(ABC):
    ...
    #         print(k, v)
    #         print(v in primitives)
    #     init_keys = set(
    #         k for k, v in cls.__init__.__annotations__.items() if v in primitives
    #     )
    #     public_attributes = set(
    #         k
    #         for k, v in self.__dict__.items()
    ...
    #     )
    #     # print(type())
    #     print("init_keys", init_keys)
    #     print("public_attributes", public_attributes)
    #     print("hidden_attributes", hidden_attributes)
    #     if init_keys <= (public_attributes | hidden_attributes):
    #         warnings.warn(
    #             f"""class {self.__name__} is missing the following attributes which are params in __init__:
    #             {init_keys ^ (public_attributes | hidden_attributes)}
