...
class LongText(Molecular):
    ...
    def chunk(self, chunk_fn: Callable[[str], List[str | tuple[str, dict] | Context]]) -> List[Context]:
        """
        Chunk the text into smaller Context objects.
        
        Args:
            chunk_fn: A function that takes in a string and returns a list of strings, string-metadata pairs, or Context objects.
        ...
        for i, chunk in enumerate(chunks):
            source = Source(type=SourceType.CONTEXT_OBJECT, origin=self, metadata={"chunk_id": i})
            if isinstance(chunk, str):
                self.chunks.append(Text(text=chunk, source=source))
            elif isinstance(chunk, tuple):
                self.chunks.append(Text(text=chunk[0], source=source, metadata=chunk[1]))
            elif isinstance(chunk, Context):
                self.chunks.append(chunk)
            else:
                raise ValueError(f"chunk_fn returned an invalid chunk type: {type(chunk)}")
        return self.chunks
