...
Modaic is a Python toolkit that helps you turn agents and retrievers into reusable packages with structured Context objects — ready to share on the Modaic Hub.
...
class EchoConfig(PrecompiledConfig):
    agent_type: str = "EchoAgent"

<Tip>
Persisted config fields must be annotated on your `PrecompiledConfig` subclass (e.g., `agent_type: str = "MyAgent"`). Unannotated attributes are not saved.
</Tip>
...
<Warning>
Pushing to or loading from the hub requires `MODAIC_TOKEN`. You can also use local paths with `local=True`.
</Warning>
...
Build structured filters with `Prop` and render them with `Filter`:

```python
from modaic.context import Prop, Filter

q = (Prop("age") >= 21) & (Prop("role") == "engineer")
print(Filter(q))
# {"$and": [{"age": {"$gte": 21}}, {"role": {"$eq": "engineer"}}]}
```
