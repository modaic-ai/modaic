...
title: Quickstart
---
Make your first DSPy program. Lets make a program that does lead qualification for an AI agent optimization company.
...
We will define a signature for our Predict program. A signature defines the schema for our program. It's instructions, the input fields, and the output fields.
Predict is a type of dspy.Module. It uses the Signature as its schema and prompts and LM to respond to a query when you call it.
```python
import dspy
...
## Optimization
DSPy lets you improve the accuracy of your programs using prompt optimization. Lets improve the accuracy of our lead qualification program using [GEPA](/dspy_guide/optimization/optimizers/GEPA).
GEPA stands for Genetic-Pareto. It is a genetic algorithm that evolves prompts using an LLM that reflects over failed examples from previous prompts. 
...
# define a metric to sscore predictions and provide feedback
