...
title: Signatures
---
Signatures act as a schema or specification for your DSPy programs. They define three key elements:
1. What the program is supposed to do (instructions)
2. The input fields 
3. The output fields

You can define signatures in two ways:
1. Class Definition
...
class USCities(dspy.Signature):
    """
    List n cities in the given state.
    """
    state: str = dspy.InputField(description="The state to get cities from")
    n: int = dspy.InputField(description="The number of cities to return", default=1)
    cities: list[str] = dspy.OutputField(description="The cities in the given state")

predict = dspy.Predict(USCities)
```
In this class definition, the docstring defines the instructions for the system prompt, and the fields tell our DSPy program how to expect inputs from the codebase and how to parse structured outputs from the language model.
When you optimize a program, the actual instructions may change, but the docstring is still useful for documentation purposes.
...
```
This is effectively the same structure as the class definition above, but without the custom instructions.
...
### Tool
Used to pass tool definitions into a signature, enabling the language model to understand available functions and their parameters. This is typically used as a list of tools in an input field.
...
**String literal:**
```python
predict = dspy.Predict("question, tools: list[Tool] -> outputs: ToolCalls")
```
