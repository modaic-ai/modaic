...
Modules define inference strategies to perform tasks. There are two main types of modules in DSPy:

**Built-in Modules -**
These are modules built into the DSPy library. They take in a signature as well as some other arguments and return a `dspy.Prediction` type containing the output fields of the base signature. Some popular built-in modules include:
- `Predict`: A simple LM call. 
- `ReAct`: A standard tool-calling agent loop.
- `ChainOfThought`: A chain-of-thought reasoning loop.

**User-Defined Modules -**
These are modules that you define yourself by subclassing `dspy.Module`. They do not need to take in a signature in their constructor, and they can use any combination of built-in and user-defined modules to perform tasks.
...
## Built-in Modules
Below are the major built-in modules in DSPy. For more, see the [Extra Modules](../utilities/modules.mdx) section.
