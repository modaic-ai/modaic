...
<Warning>
  There needs to be exactly two output fields in your signature for an arbiter,
  one the shape of the output (as a builtin type or a pydantic model) and one
  the ``reasoning`` field. Input fields can be of any type and amount.
</Warning>
