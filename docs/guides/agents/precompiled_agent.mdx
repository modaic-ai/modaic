...
The `PrecompiledAgent` class is the bread and butter of Modaic. It is the home of your agent's logic. You can think of it alot like the `PretrainedModel` class in huggingface's transformers library. `PrecompiledAgent` is a subclass of `dspy.Module` from the [Dspy](https://dspy.ai/) library. So anything you can define with a `dspy.Module` you can define with a `PrecompiledAgent`. 
What `PrecompiledAgent` adds to `dspy.Module` is direct integrate with Modaic's context engineering, observability, and version control stack.
...
You can load an agent from Modaic Hub with the `from_precompiled()` method. This will load the agent's prompts and config into your local environment. To load the entire agent, including python modules and third party dependencies, see the [AutoAgent](/guides/agents/auto_agent) guide.
