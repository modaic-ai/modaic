...
sidebarTitle: Arbiters + Braintrust
---
...
[Instructions to create an arbiter here](/docs/arbiters/create_an_arbiter).

## Set the MODAIC_TOKEN

In Braintrust, click on the "Settings" tab at the bottom of the sidebar menu
![Settings Tab](https://d1pz4mbco29rws.cloudfront.net/public/docs/docs/arbiters/braintrust/settings.png)

In the Settings menu, click on the "Env variables" tab and add your `MODAIC_TOKEN` as an environment variable.
![Env Variables](https://d1pz4mbco29rws.cloudfront.net/public/docs/docs/arbiters/braintrust/env_variables.png)
...
def correctness_scorer(input: dict, output: dict):
    ...
    return result.output.score # output of the arbiter will always be in the `output` field.
...
<Warning>
  Your arbiter must accept the schema you pass into `predict`. In this case the
  `modaic/correctness` arbiter accepts the fields `input: dict` and `output:
